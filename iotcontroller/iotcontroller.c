...
// data output pin. 19 PB1 -> DI PI 23
// data input pin.  20 PB2 <- DO PI 27
...
void internalSpiInit(void) {
  ...
  PUEC  = 0b00000010;   // PC1 (SCL) pull up
...
// functions for the master spi (pi or similar)

void externalSpiInit(void) {
  // Configure port directions.
  DDRB =  0b00000100; // PB1 input (mosi) and PB2 output (miso)
  PORTB = 0b00000000; //  
  PUEB =  0b00000010; //  
...
void externalSpiPut(unsigned char byte) {
  USIDR = byte;
  USICR = 0b00011000;
  USISR = 0b01000000;
  while ( (USISR & 0b01000000) != 0b01000000 ){}
